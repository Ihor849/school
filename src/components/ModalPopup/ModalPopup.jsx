...
const ModalPopup = ({ isOpen, onClose, item }) => {
  ...
  return createPortal(
    <ModalOverlay
      className={`modal ${
        isOpen ? ['active', 'modal-backdrop'].join(' ') : 'modal-backdrop'
      }`}
    >
      <ModalContainer>
        ...
        <CarDescription>
          ...
          <RowWrapper>
            <p>
              Fuel Consumption: <span>{fuelConsumption}</span>
            </p>
            <p>
              Engine Size: <span>{engineSize}</span>
            </p>
          </RowWrapper>
        ...
        <FlatButton className="modal-close" onClick={shut}>
          <RxCross2 />
        </FlatButton>
      </ModalContainer>
    </ModalOverlay>,
